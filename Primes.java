...
//Generate and store a list of primes from a given starting point.
	...
	public void generateTwinPrimes()
	{
		...
		{
			if (primeList.get(i+1).equals((primeList.get(i).add(new BigInteger("2")))) )
			{
	...
	public void generateHexPrimes()
	{
		...
		for (int i=0; i < twinPrimeList.size()-1; i++)
		{
			...
			for (int j=i+1; j < twinPrimeList.size(); j++)
			{
				...
				if (n.multiply(new BigInteger("2")).equals(twoN) )
				{
	...
	{
		if(primeList.size()!=0) {
			String prime = primeList.get(primeList.size()-1).toString();
			return prime.length(); 
		}
		else {
			return 0;
	...
	{
		if(crossList.size()!=0) {
			Pair<BigInteger> cross = crossList.get(crossList.size()-1);
			Integer l = cross.left().toString().length();
			Integer r = cross.right().toString().length();
			Pair<Integer> answer = new Pair<Integer>(l,r);
			return answer;
		}
		else {
